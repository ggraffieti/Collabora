...
import org.gammf.collabora.communication.messages.{PublishCollaborationInCollaborationExchange, PublishNotificationMessage}
  ...
  override def receive: Receive = {
    ...
    case QueryOkMessage(queryGoneWell) => queryGoneWell match {
      case query: QueryCollaborationMessage => query match {
        case _: InsertCollaborationMessage => collaborationMemberActor ! PublishCollaborationInCollaborationExchange(query.userID, CollaborationMessage(user=query.userID,collaboration = query.collaboration))
