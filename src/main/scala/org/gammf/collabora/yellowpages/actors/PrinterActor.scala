...
object HierarchyTest extends App {
  ...
  val printer2 = system.actorOf(PrinterActor.printerProps(yellowPages = root, topic = Topic() :+ Communication :+ RabbitMQ :+ Database))
  ...
  val topic3 = system.actorOf(YellowPagesActor.topicProps(yellowPages = root, name = "Communication/Http_YP", topic = Topic() :+ Communication))
